...
CRAWL_PAGE_THREADINGS = 2
PARSE_PAGE_THREADINGS = 2
CRAWL_ARTICLE_THREADINGS = 4
PARSE_ARTICLE_THREADINGS = 4
BASE_URL = "https://www.adreep.cn"
index = "www.adreep.cn"
...
class ParseArticleThread(Thread):
    ...
    def parse(self, item):
        url = item.url
        selector = HTML(item.text)
        # https://www.adreep.cn/dxyy/52897.html
        article_id = url.rsplit("/", 1)[-1].split(".")[0]
        title = selector.xpath("//h1/text()")[0]
        date = selector.xpath('//span[@class="date"]/text()')[0]
        category = selector.xpath('//ol/li[position()=2]/a/@title')[0]
        bodybox = selector.xpath('//div[@id="bodybox"]')[0]
...
def parse_index(url):
    ...
    category_tags = selector.xpath('//li[@class="nav-item"]/a')
    category_urls = []
    for tag in category_tags:
        if tag.xpath('./text()')[0] != '专题':
            partial = tag.xpath('./@href')[0]
            category_urls.append(urljoin(BASE_URL, partial))
...
def parse_category(url):
    ...
    response = fetch(url)
    selector = HTML(response.text)
    last_page = selector.xpath('//div[@class="thisclass"]/a[position()=last()]/@href')[0]
    total_num = int(last_page.rsplit("=", 1)[-1])
    return [urljoin(url, f'?page={i}') for i in range(1, total_num + 1)]
...
if __name__ == '__main__':
    ...
    # parse_article("https://www.adreep.cn/dxyy/56980.html")
